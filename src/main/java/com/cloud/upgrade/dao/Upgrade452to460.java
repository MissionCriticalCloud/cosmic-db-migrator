...
public class Upgrade452to460 implements DbUpgrade {
    final static Logger s_logger = LoggerFactory.getLogger(Upgrade452to460.class);
    ...
    public File[] getPrepareScripts() {
        ...
        if (script == null) {
            throw new GeneralException("Unable to find db/schema-452to460.sql");
        }
    ...
    public File[] getCleanupScripts() {
        ...
        if (script == null) {
            throw new GeneralException("Unable to find db/schema-452to460-cleanup.sql");
        }
                ...
                try (PreparedStatement selectUserStatement = conn.prepareStatement(userSql)) {
                    ...
                    if (userResults.next()) {
                        ...
                        try (PreparedStatement updateStatement = conn.prepareStatement(userIdUpdateSql)) {
                            ...
                            throw new GeneralException("Unable to update user ID " + userId + " on vm_instance id=" + vmId, e);
                        }
                    }
                } catch (final SQLException e) {
                    throw new GeneralException("Unable to update user ID using accountId " + accountId + " on vm_instance id=" + vmId, e);
                }
            }
        } catch (final SQLException e) {
            throw new GeneralException("Unable to update user Ids for previously deployed VMs", e);
        }
    ...
    private void addIndexForVMInstance(final Connection conn) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement("ALTER TABLE `cloud`.`vm_instance` ADD INDEX `i_vm_instance__instance_name`(`instance_name`)")) {
            ...
            throw new GeneralException("Unable to add index i_vm_instance__instance_name to vm_instance table for the column instance_name", e);
        }
    }
    ...
    private void addRedundancyForNwAndVpc(final Connection conn) {
        ...
        try (PreparedStatement addRedundantColToVpcOfferingPstmt = conn
            ...
            e.printStackTrace();
            throw new GeneralException("Adding redundancy to vpc, networks and vpc_offerings failed", e);
        }
    }
