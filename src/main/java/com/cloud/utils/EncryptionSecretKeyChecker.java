    ...
    public void check(final Properties dbProps) throws IOException {
        ...
        if (encryptionType.equals("file")) {
            ...
            if (is == null) {  //This is means we are not able to load key file from the classpath.
                throw new GeneralException(s_keyFile + " File containing secret key not found in the classpath: ");
            }
            ...
            try (BufferedReader in = new BufferedReader(new InputStreamReader(is))) {
                ...
                throw new GeneralException("Error while reading secret key from: " + s_keyFile, e);
            }

            if (secretKey == null || secretKey.isEmpty()) {
                throw new GeneralException("Secret key is null or empty in file " + s_keyFile);
            }
        } else if (encryptionType.equals("env")) {
            secretKey = System.getenv(s_envKey);
            if (secretKey == null || secretKey.isEmpty()) {
                throw new GeneralException("Environment variable " + s_envKey + " is not set or empty");
            }
            ...
            try (ServerSocket serverSocket = new ServerSocket(port)) {
                ...
                try (
                    ...
                    throw new GeneralException("Accept failed on " + port);
                }
            } catch (final IOException ioex) {
                throw new GeneralException("Error initializing secret key reciever", ioex);
            }
        } else {
            throw new GeneralException("Invalid encryption type: " + encryptionType);
        }
