...
public class Upgrade442to450 implements DbUpgrade {
    ...
    public File[] getPrepareScripts() {
        ...
        if (script == null) {
            throw new GeneralException("Unable to find db/schema-442to450.sql");
        }
    ...
    public File[] getCleanupScripts() {
        ...
        if (script == null) {
            throw new GeneralException("Unable to find db/schema-442to450-cleanup.sql");
        }
    ...
    private void updateMaxRouterSizeConfig(final Connection conn) {
        ...
        try (PreparedStatement updatePstmt = conn.prepareStatement(sqlUpdateConfig)) {
            ...
            throw new GeneralException("Unable to upgrade max ram size of router in config.", e);
        } catch (final UnsupportedEncodingException e) {
            throw new GeneralException("Unable encrypt configuration values ", e);
        }
    ...
    private void upgradeMemoryOfVirtualRoutervmOffering(final Connection conn) {
        ...
        try (
            ...
            throw new GeneralException("Unable to upgrade ram_size of service offering for domain router. ", e);
        }
    ...
    private void upgradeMemoryOfInternalLoadBalancervmOffering(final Connection conn) {
        ...
        try (PreparedStatement selectPstmt = conn.prepareStatement("SELECT id FROM `cloud`.`service_offering` WHERE vm_type='internalloadbalancervm'");
            ...
            throw new GeneralException("Unable to upgrade ram_size of service offering for internal loadbalancer vm. ", e);
        }
